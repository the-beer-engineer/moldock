...
if (!picked) { console.log('no UTXOs'); process.exit(0); }
...
// Unique OP_RETURN: pad nonce to even length
const nonce = Date.now().toString(16).padStart(16, '0');
tx.addOutput({ lockingScript: Script.fromHex(`006a08${nonce}`), satoshis: 0 });
tx.addOutput({ lockingScript: new P2PKH().lock(addr), satoshis: picked.value - 500 });
await tx.sign();
console.log(`tx: ${tx.id('hex')} size=${tx.toHex().length/2}B`);
...
for (let i = 0; i < 3; i++) {
  ...
  console.log(`attempt ${i+1}: ${j.txStatus || 'X'} ${(j.extraInfo || j.error || '').slice(0, 200)}`);
  if (j.txStatus !== 'REJECTED' && !j.error) break;
